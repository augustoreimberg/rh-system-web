...
export async function getEmployees(): Promise<{
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/employes?populate=filial`,
      {
        cache: "no-store",
      }
    );
...
export async function createEmployee(
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/employes`,
      {
        method: "POST",
        headers: {
...
export async function updateEmployee(
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/employes/${documentId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
